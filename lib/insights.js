// lib/insights.js - Server-side safe insights aggregator

// Helper function to safely get data from localStorage (only in browser)
...
// Server-safe version that accepts data as parameters
export async function getInsights(userData = null) {
  // If we're in the server and no user data provided, return empty data
  ...
  // If we have user data, use it (server-side with provided data)
  if (userData) {
    // Use provided user data
    const { activities = [], meals = [], sleepData = [], session } = userData;
    const today = new Date();
    const weekStart = getWeekStart(today);
    const todayStr = formatDate(today);
    
    // Process data similar to client-side logic
    ...
    // For server-side, we can't access localStorage, so we approximate steps
  ...
  // If we're in the browser, use localStorage as before
  // This is for backward compatibility, but we should use insightsClient.js instead
  console.warn("Using deprecated localStorage-based insights in browser. Consider using insightsClient.js instead.");
  
  // Get session (this will fail in server)
  try {
    ...
    const activityKey = getUserKey('activities', session.username);
    const mealKey = getUserKey('meals', session.username);
    ...
    const sleepKey = getUserKey('sleep', session.username);
    let sleepData = [];
    for (let i = 0; i < 7; i++) {
...
// Function to calculate summary statistics for AI prompt (server-safe)
export async function getSummaryStats(userData = null) {
